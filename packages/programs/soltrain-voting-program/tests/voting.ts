...
describe('voting', () => {
    ...
    describe('> createVotingSession', () => {
        it('> should succeed when called with program administrator account', async () => {
            ...
            const { tx, accounts: { sessionAccountPubkey } } =
    ...
    describe('> Voting actions are conditionned by voting session status', () => {
        ...
        beforeEach(async () => {
            const { accounts: { sessionAccountPubkey } } = await client.createVotingSession(
                administrator.payer,
                'Super Heroes',
        ...
        context('## voting status is RegisteringVoters', () => {
            ...
            describe('> registerVoter', () => {
                it('> should succeed when called with voting session administrator', async () => {
                    const { tx, accounts: { voterAccountPubkey } } = await client.registerVoter(administrator.payer, sessionId, superman.publicKey);
                ...
                it('> should fail when voter address is owner contract address', async () => {
                    try {
                        await client.registerVoter(administrator.payer, sessionId, administrator.publicKey);
                        assert.ok(false);
                    } catch (_err) {
                        assert.isTrue(_err instanceof AnchorError);
                        const err: AnchorError = _err;
                        assert.match(err.toString(), /AdminForbiddenAsVoter/);
                        assert.strictEqual(err.error.errorMessage, 'Voting session administrator can not be registered as voter');
