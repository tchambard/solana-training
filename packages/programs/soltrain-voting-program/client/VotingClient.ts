import { AnchorError, BN, BorshCoder, EventParser, Program } from '@coral-xyz/anchor';
...
export class VotingClient {
    ...
    constructor(program: Program<Voting>, options?: IVotingClientOptions) {
        ...
        this.globalAccount = PublicKey.findProgramAddressSync([Buffer.from('global')], program.programId)[0];
    ...
    public async initProgram(payer: Keypair,) {
    ...
    public async createVotingSession(payer: Keypair, name: string, description: string) {
        ...
        const sessionAccount = await this.program.account.sessionAccount.fetch(sessionPubKey);
        const createdSessionEvents = await this.getTxEvents(createSessionTx);

        return { createSessionTx, sessionPubKey, createdSessionEvents, sessionAccount };
    }

    public async getNextSessionId(): Promise<BN> {
        return (await this.program.account.globalAccount.fetch(this.globalAccount)).sessionCount
    }
